<?php
ob_start();
define('API_KEY','294046963:AAFUc-ROg3m_Tym70Lb-2wXvsuMdCGL14TA');
$the_admin_id = 119296662;

file_put_contents("count",file_get_contents("count")+1);

$ad_btn='آموزش های رایگان برنامه نویسی';
$ad_url='http://telegram.me/KING_CH';
$ad_text = "با ما به راحتی برنامه نویسی را یاد بگیرید";
                ...
                'results' => json_encode([[
                    ...
                    [
                        ...
                        'input_message_content'=>[
                            'message_text'=>'@SCCROBOT getlist-'.$theListId
                        ]
                    ]])
            ]));
        }else{
...
<pre>@SCCROBOT d6cdbea45b238632bdd6d11dcf7fe98f </pre>
...
<code>@SCCROBOT getlist-XXXXX </code>
...
👈  حالا برای من کد inline تان را بفرستید : ( فعلا فقط لیست ها قبول هستند )".$cancel_Text,
                ...
                case '2':{
                    ...
                    if(str_replace('@SCCROBOT getlist-','',$code) != $code){
                        $code_list = trim(str_replace('@SCCROBOT getlist-','',$code));
...
اگر بد توضیح دادم روی /help کلیک کن راهنمایی مفصل داره !'
                            ...
                            $theCommand = '@SCCROBOT getlist-'.ListTab::getCurrentMakeID($update->message->from->id);
                            makeHTTPRequest('sendMessage',[
                                'chat_id'=>$update->message->from->id,
                                'text'=>$theCommand,
                                'reply_markup'=>json_encode([
...
اگر بد توضیح دادم روی /help کلیک کن راهنمایی مفصل داره !'
                    ...
                    $theCommand = '@SCCROBOT getbanner-'.getCurrentMakeID($update->message->from->id);
                    makeHTTPRequest('sendMessage',[
                        'chat_id'=>$update->message->from->id,
                        'text'=>$theCommand,
                        'reply_markup'=>json_encode([
...
//                    ]);
//                    makeHTTPRequest('sendMessage',[
//                        'text'=>'@SCCROBOT getbanner-'.getCurrentMakeID($userID),
//                        'chat_id'=>$userID
//                    ]);
//                }break;
                ...
                case '9':{
                    ...
                    if(is_valid_url($userTEXT)) {
                        ...
                        makeHTTPRequest('sendMessage',[
                            'text'=>"شناسه کانال را بدهید. مانند @KING_CH\n\nاگر کانال شناسه ندارد کامند /skipstep  را بزنید\nجهت انصراف ، کامند /cancel را وارد کنید",
                            'chat_id'=>$userID
                        ]);
                    }else{
                        makeHTTPRequest('sendMessage', [
